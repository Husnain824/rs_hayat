...
function initRevealObserver() {
  ...
  const isPefPage = Boolean(document.querySelector(".pef-wrap"));
  const observer = new IntersectionObserver((entries) => {
    entries.forEach((entry) => {
...
function initContactFormRateLimit() {
  const form = document.getElementById('contactForm');
  ...
  checkAndUpdateButtonState(email, sendBtn);
  
  // Email change par check karo
  document.querySelector('[name="email"]').addEventListener('change', function() {
    checkAndUpdateButtonState(this.value, sendBtn);
  });
  
  form.addEventListener('submit', function(e) {
    const email = document.querySelector('[name="email"]').value;
    const storageKey = 'msgs_' + email;
    const now = Date.now();
    const maxMsgs = 2;
    const timeWindow = 24 * 60 * 60 * 1000; // 24 hours

    let data = JSON.parse(localStorage.getItem(storageKey) || '{"count":0,"time":0}');
    
    // Agar 24 hours ho gaye to reset kar do
    if (now - data.time > timeWindow) {
      data = {"count":0,"time":now};
    }
    
    // Limit check karo
    if (data.count >= maxMsgs) {
      e.preventDefault();
      e.stopPropagation();
      e.stopImmediatePropagation();
      alert(`Aap ne 24 hours mein ${maxMsgs} messages bhej chuke hain. Kal dobara try karein.`);
      sendBtn.disabled = true;
      sendBtn.textContent = '⏱️ 24 GHANTE BAAD RETRY KARO';
      return false;
    }
...
function checkAndUpdateButtonState(email, button) {
  const storageKey = 'msgs_' + email;
  const now = Date.now();
  const maxMsgs = 2;
  ...
  if (data.count >= maxMsgs) {
    ...
    button.textContent = '⏱️ 24 GHANTE BAAD RETRY KARO';
  } else {
